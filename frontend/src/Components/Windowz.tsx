...
export default function Windowz(props: WindowzProps) {
    ...
    const [relPos, setRelPos] = useState({ x: 0, y: 0 }); // Relative position of cursor to the element
    ...
    const renderDragHandle = () => {
        return <div onMouseDown={onMouseDown} className="windowz-title-move">
            {/* //@ts-ignore */}
    ...
    const renderContentFrame = (innards: any) => {
        return <div style={{
    ...
    const renderFrame2 = (innards: any) => {
        return <div
            style={{
                position: 'absolute',
                left: `${position.x}px`,
        ...
        >
            {renderContentFrame(innards)}
        </div>
    }

    return renderFrame2(props.children)
}
